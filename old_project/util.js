...
export function gameOverModal(grid, miniGrid, completeNodes) {
  ...
  for (let i = 0; i < grid.length; i++) {
    for (let j = 0; j < grid[0].length; j++) {
      ...
      let tile = null;
      let [div, completeCell, deadCell, cellTotalPoints] = wordPaths(
        rootNode,
        miniGrid,
        completeNodes
      );
  ...
  totalStagePoints.innerHTML =
    window.gamePoints + totalBonusPoints + timeBonus * 1000;
