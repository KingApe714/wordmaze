...
export function gridNode(coordinates) {
  ...
  let letter = "AABCDEEFGHIIJKLMNOOPQRSSTUUVWXYZ"[
...
//   let definition = json[0]
//     ? json[0].meanings[0].definitions[0].definition
//     : "Sorry, no definition for this one, but we know it's a word..";
    ...
    if (currentNode.complete) {
      ...
      clueWordContainer.addEventListener("mousedown", () => {
        ...
        for (let i = 0; i < innerClueContainer.children.length; i++) {
          ...
          let childNodes =
            innerClueContainer.children[i].querySelectorAll(".clue-letter");
...
export function setUpTiles(grid, gameWords, completeNodes) {
  ...
  gameBoardContainer.addEventListener("mouseup", () => {
    ...
    if (!nodeAdam.found && nodeAdam.complete && gameWords.includes(word)) {
      ...
      completeNodes[firstNode] = completeNodes[firstNode]
        .slice(0, idx)
        .concat(completeNodes[firstNode].slice(idx + 1));
      ...
      nodeAdam.clueWordContainer.querySelector(
        ".clue-word-shadow"
      ).style.backgroundColor = "transparent";

      //green highlight found clue word
      nodeAdam.clueWordContainer.style.backgroundColor =
        "rgba(0, 230, 65, 0.85)";
      ...
      if (!completeNodes[firstNode].length) {
        ...
        grid[y][x].node.innerTile.style.backgroundColor =
          "rgba(0, 230, 65, 0.45)";
      ...
      wordExpression.style.left = `${
        wordContainer.getBoundingClientRect().left
      }px`;
      ...
      top = top > 750 ? 750 : top;
      wordExpression.style.left = `${
        nodeAdam.clueWordContainer.getBoundingClientRect().left
      }px`;
      ...
      gNode.innerTile.addEventListener("mousemove", () => {
        ...
        if (mouseDown) {
          ...
          if (!gNode.selected) {
            ...
            let [x1, y1] =
              selectedNodes[selectedNodes.length - 1].coordinates.split(",");
            ...
            if (
              nodeAdam &&
              nodeAdam.children[gNode.coordinates] &&
            ...
            if (selectedNodes.includes(gNode)) {
              ...
              for (let i = selectedNodes.length - 1; i >= 0; i--) {
                ...
                if (currentNode === gNode) {
                  ...
                  currentNode.innerTileContainer.classList.remove(
                    "selected-inner-tile"
                  );
                  currentNode.innerTile.classList.remove(
                    "selected-inner-tile-shrink"
                  );
                  currentNode.innerTile.classList.remove(
                    "selected-inner-tile-grow"
                  );
                  ...
                  if (!completedTiles.includes(currentNode)) {
                    ...
                    currentNode.innerTile.style.backgroundColor =
                      "rgba(0, 230, 65, 0.45)";
                  }
...
export function findWords(gridNode, tree) {
  ...
  while (queue.length) {
    ...
    for (let i = 0; i < ele[1].node.neighbors.length; i++) {
      if (
        !visitedNodes.includes(ele[1].node.neighbors[i]) &&
        ...
        if (subTree.map[char]) {
          ...
          currentNode.children[ele[1].node.neighbors[i].coordinates] =
            new ancestoryNode(ele[1].node.neighbors[i]);
          let newAncNode =
            currentNode.children[ele[1].node.neighbors[i].coordinates];
